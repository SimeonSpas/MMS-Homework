...
int main(){
    ...
    //qsort(books, COUNT, sizeof(*books), comparePagesDESC);
     Books_t key = {.title = "Do moeto purvo libe", .author = "Hristo Botev", .pages = 200, .price = 20.50};
    Books_t *element = linearSearch(&key, books, COUNT, sizeof(*books),comparePriceASC);
    if (!element){
        printf("Not found!\n");
    }else{
        printf("Found!\n");
